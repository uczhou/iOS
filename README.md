...
### 1. 首先还是对微博相册进行分析
本次微博相册爬虫爬取的对象是某大V，按照其他文章所写，现在爬取微博图片（或者说社交网络数据）首先考虑移动端。所以这次就直接用Chrome的Device mode加载微博页，然后用Chrome自带的Developer Tools抓取网页加载。其中有一个向m.weibo.cn/api/container发送的请求。
